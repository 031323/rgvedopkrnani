...
class crnm(krmh):
	...
	def __next__(self):
		self.i += 1
		if self.i <= len(self.r[self.c*2-1].getchildren()):
			return self[self.i]
		raise StopIteration

class rk(krmh, root):
	def strata(self):
		return self.r[1][0][2][1].text
	def __getitem__(self, c):
		return crnm(self.r[0], c)
	def __next__(self):
		self.i += 1
		if self.i * 2 <= len(self.r[0].getchildren()):
			return self[self.i]
		raise StopIteration
...
class suktm(krmh, root):
	...
	def __next__(self):
		self.i += 1
		if self.i <= len(self.r.getchildren()) - 1:
			return self[self.i]
		raise StopIteration
...
class mndlm(krmh, root):
	...
	def __next__(self):
		self.i += 1
		if self.i <= len(self.r[1][0][0].getchildren()):
			return self[self.i]
		raise StopIteration
...
r = rv()
strata = set()
print(r[8][2][20][2].smhita())
print(r[8][2][20][2][2].drmani())